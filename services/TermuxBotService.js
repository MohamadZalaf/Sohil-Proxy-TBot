...
const TERMUX_DIR = '/data/data/com.termux/files/home/ProxyBot/';
...
class TermuxBotService {
  ...
  async checkTermuxInstalled() {
    try {
      ...
      const result = await exec('pm list packages | grep com.termux');
      return result.includes('com.termux');
    } catch (error) {
      return false;
    }
  }

  async installBotFiles() {
    try {
      console.log('📁 إنشاء مجلد البوت...');
      
      // إنشاء مجلد البوت
      await FileSystem.makeDirectoryAsync(BOT_FILES_DIR, { intermediates: true });

      // إنشاء ملف البوت Python
...
echo "🚀 بدء تشغيل بوت البروكسي على Termux..."
...
`;
  ...
  async setupTermux() {
    try {
      console.log('🔧 إعداد Termux...');

      // نسخ الملفات إلى Termux
      await exec(`mkdir -p ${TERMUX_DIR}`);
      await exec(`cp -r ${BOT_FILES_DIR}* ${TERMUX_DIR}`);
      await exec(`chmod +x ${TERMUX_DIR}start_bot.sh`);

      console.log('✅ تم إعداد Termux بنجاح');
      return true;
    } catch (error) {
      console.error('❌ خطأ في إعداد Termux:', error);
      return false;
    }
  }
  ...
  async startBot() {
    try {
      ...
      // التحقق من Termux
      const termuxInstalled = await this.checkTermuxInstalled();
      if (!termuxInstalled) {
        Alert.alert(
          'Termux مطلوب',
          'يجب تثبيت Termux لتشغيل البوت. هل تريد تنزيله؟',
          [
            { text: 'إلغاء', style: 'cancel' },
            {
              text: 'تنزيل',
              onPress: () => {
                // فتح متجر التطبيقات لتنزيل Termux
                exec('am start -a android.intent.action.VIEW -d "https://play.google.com/store/apps/details?id=com.termux"');
              }
            }
          ]
        );
      ...
      // إنشاء ملفات البوت
      const filesCreated = await this.installBotFiles();
      if (!filesCreated) {
        throw new Error('فشل في إنشاء ملفات البوت');
      }

      // إعداد Termux
      const termuxSetup = await this.setupTermux();
      if (!termuxSetup) {
        throw new Error('فشل في إعداد Termux');
      }

      // تشغيل البوت في Termux
      const startCommand = `am start -n com.termux/.HomeActivity && sleep 2 && input text "cd ProxyBot && ./start_bot.sh" && input keyevent 66`;
      ...
      setTimeout(() => {
        Alert.alert(
          'تم بدء البوت! 🚀',
          'البوت يعمل الآن على Termux في الخلفية.\n\n• تأكد من إبقاء Termux يعمل\n• البوت سيستقبل الطلبات تلقائياً\n• يمكنك إغلاق هذا التطبيق بأمان',
          [{ text: 'موافق' }]
        );
      }, 3000);
      ...
      Alert.alert('خطأ', error.message || 'فشل في بدء البوت');
      return false;
    }
  }
  ...
  async stopBot() {
    try {
      if (!this.isRunning) {
        throw new Error('البوت متوقف بالفعل');
      }

      console.log('⏹️ إيقاف البوت...');

      // إيقاف البوت في Termux
      const stopCommand = `am start -n com.termux/.HomeActivity && sleep 1 && input keyevent 3`; // Ctrl+C
      await exec(stopCommand);
